...
exit ("Извините, введённый вами логин или пароль неверный."); //останавливаем выполнение сценариев
          ...
          $_SESSION['passwordadmin']=$myrow['password']; 
		  $_SESSION['loginadmin']=$myrow['login']; 
          $_SESSION['idadmin']=$myrow['id'];//эти данные очень часто используются, вот их и будет "носить с собой" вошедший пользователь
...
setcookie("loginadmin", $_POST["login"], time()+9999999);
setcookie("passwordadmin", $_POST["password"], time()+9999999);
setcookie("idadmin", $myrow['id'], time()+9999999);
}

if (isset($_POST['autovhod'])){
//Если пользователь хочет входить на сайт автоматически
setcookie("autoadmin", "yes", time()+9999999);
setcookie("loginadmin", $_POST["login"], time()+9999999);
setcookie("passwordadmin", $_POST["password"], time()+9999999);
setcookie("idadmin", $myrow['id'], time()+9999999);}
}	

echo "<html><head><meta http-equiv='Refresh' content='0; URL=$uri'></head></html>";
