...
<!--<tr><td><a style="font-size:20px;text-decoration:none;" >Изображение 2 (750x500 px): </a></td><td>		<input id="xFilePath2" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img2" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img2']));?>" />
		<input type="button" value="Выбрать" onclick="BrowseServer2();" /></td>
</tr>
<tr><td><a style="font-size:20px;text-decoration:none;" >Изображение 3 (750x500 px): </a></td><td>		<input id="xFilePath3" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img3" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img3']));?>" />
		<input type="button" value="Выбрать" onclick="BrowseServer3();" /></td>
</tr>
<tr><td><a style="font-size:20px;text-decoration:none;" >Изображение 4 (750x500 px): </a></td><td>		<input id="xFilePath4" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img4" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img4']));?>" />
		<input type="button" value="Выбрать" onclick="BrowseServer4();" /></td>
</tr>
<tr><td><a style="font-size:20px;text-decoration:none;" >Изображение 5 (750x500 px): </a></td><td>		<input id="xFilePath5" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img5" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img5']));?>" />
...
function complete(){ 
	 ...
	 $offer_created_id = mysql_insert_id($db);
	  ...
	  echo "<meta http-equiv='Refresh' content='0; URL=money_offer.php?city=".$_POST['city_id']."'>"; 
} 
function show_pages() { 
?> 
...
if (($_GET['city']) && ($_GET['city'] > 0)) {
	...
	// Sorting
		...
		if ($sortfield && $sortorder) {
		 ...
		 $result = mysql_query("SELECT b.id as id, b.draft as draft, b.name as name, b.date as date,b.date1,b.placement_date, b.protected, a.name as adv_name, a.draft as adv_draft, c.name as city_name, b.author,b.modifiedby FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE b.deleted=0 AND cat_id=6 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
		
		$result_hidden_offers = mysql_query("SELECT b.id FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE b.draft=1 AND b.deleted=0 AND cat_id=6 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
		$result_hidden_by_adv = mysql_query("SELECT b.id FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE (b.draft=1 OR a.draft=1) AND b.deleted=0 AND cat_id=6 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
...
<tr class="table-head"> 
 <td style="width:50px;">ID</td>
  ...
  <td><a href="/admin/money_offer.php?city='.$_GET['city'].$sortlink.'">Срок размещения '.$sorticon.'</a></td>
     <td>Кем создано</td>
   <td>
 Создано
  </td>
						...
						<!-- Nav -->
							<nav id="nav">
								<ul>
									...
									<li class="current_page_item"><a class="modifiable-link" href="money_offer.php">Деньги</a></li>
