...
function BrowseServer()
{
	...
	finder.basePath = '../resource/ckfinder/';	// The path for the installation of CKFinder (default = "/ckfinder/").
...
function validateOfferForm() {
	...
	if (validateField("select","offer_cat_id","Категория") && validateField("select","advertiser_id","Рекламодатель") && validateDates()) {
...
<?if (($row['offer_cat_id'] == '') ||($row['offer_cat_id'] == 0)){
		echo "<option value=0>(не определена)</option>";
	} 
?>
<? $sqlc = mysql_query("SELECT * FROM offercategories WHERE section_id = ".$OFFERS_SECTION_ID." AND deleted=0 ORDER BY disp_order ASC");
...
<a style="font-size:20px;text-decoration:none;" >Рекламодатель:</a> </td><td>
...
?>
	...
	<a style="font-size:20px;text-decoration:none;" >Миниатюра : </a></td><td><img  id="xImg" src="" class="srf_icon_preview"/>		<input id="xFilePath" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img']));?>" />
...
<a style="font-size:20px;text-decoration:none;" >Изображение 1 : </a></td><td>		<input id="xFilePath1" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img1" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img1']));?>" />
		<input type="button" value="Выбрать" onclick="BrowseServer1();" /></td>
</tr>
<tr><td>
<img  id="xImg2" src="" class="srf_thumb_preview"/>
<a style="font-size:20px;text-decoration:none;" >Изображение 2 : </a></td><td>		<input id="xFilePath2" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img2" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img2']));?>" />
		<input type="button" value="Выбрать" onclick="BrowseServer2();" /></td>
</tr>
<tr><td>
<img  id="xImg3" src="" class="srf_thumb_preview"/>
<a style="font-size:20px;text-decoration:none;" >Изображение 3 : </a></td><td>		<input id="xFilePath3" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img3" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img3']));?>" />
		<input type="button" value="Выбрать" onclick="BrowseServer3();" /></td>
</tr>
<tr><td>
<img  id="xImg4" src="" class="srf_thumb_preview"/>
<a style="font-size:20px;text-decoration:none;" >Изображение 4 : </a></td><td>		<input id="xFilePath4" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img4" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img4']));?>" />
		<input type="button" value="Выбрать" onclick="BrowseServer4();" /></td>
</tr>
<tr><td>
<img  id="xImg5" src="" class="srf_thumb_preview"/>
<a style="font-size:20px;text-decoration:none;" >Изображение 5 : </a></td><td>		<input id="xFilePath5" style="padding:5px; background-color:#e6e6e6;font-size:15px;" name="img5" type="text" size="50" value="<?=htmlspecialchars(stripslashes($row['img5']));?>" />
	 ...
	 <tr><td><a style="font-size:20px;text-decoration:none;" >Размер скидки:</a></td> 
      <td> <input style="padding:5px; background-color:#e6e6e6;font-size:15px;" type="text" name="discount_size" class="enter" size="75" value="<?=htmlspecialchars(stripslashes($row['discount_size']));?>"> </td></tr>
...
function complete(){ 
      ...
      if(empty($row['id'])) 
                     ...
                     (offer_cat_id,
                      ...
                      cat_id
                            ...
                            ('".mysql_real_escape_string($_POST['offer_cat_id'])."', 
							 ...
							 '".((($_POST['system_date_to'] == NULL) || ($_POST['system_date_to'] =='0')) ? 'NULL' : mysql_real_escape_string($_POST['system_date_to']))."',
            ...
            $query = "UPDATE blog SET 
									 ...
									 system_date_to = '".((($_POST['system_date_to'] == NULL) || ($_POST['system_date_to'] =='0')) ? 'NULL' : mysql_real_escape_string($_POST['system_date_to']))."',
      ...
      echo "<meta http-equiv='Refresh' content='0; URL=post.php?city=".$_POST['city_id']."'>"; 
} 
function show_pages() { 
?> 
...
<? 
		...
		<h3><a  class="modifiable-link" href="?id=new"><input style="border-radius: 0.4em;border: solid 1px #ddd;padding: 0.5em;" type="submit" value="Добавить предложение" ></a></h3><br> 
...
// City filter
        ...
        $result = mysql_query("SELECT b.id as id, b.draft as draft, b.name as name, b.date as date,b.date1,b.paid, b.placement_date, b.protected, a.name as adv_name,a.id as advid, a.draft as adv_draft, c.name as city_name, b.author,b.modifiedby FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE b.deleted=0 AND cat_id=1 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
		
		$result_hidden_offers = mysql_query("SELECT b.id FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE b.draft=1 AND b.deleted=0 AND cat_id=1 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
		$result_hidden_by_adv = mysql_query("SELECT b.id FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE (b.draft=1 OR a.draft=1) AND b.deleted=0 AND cat_id=1 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
...
<tr class="table-head"> 
  ...
  <td>Платный</td>
  <td><a href="/admin/post.php?city='.$_GET['city'].$sortlink.'">Срок размещения '.$sorticon.'</a></td>
  <td>Кем создано</td>
  <td>Создано  </td>
  <td>Кем изменено</td>
   <td>Изменено</td>
...
function delete_pages(){ 
        ...
        mysql_query($query, $db); 
        echo '<h3>Предложение удалено</h3>'; 
		...
		<!-- Sidebar -->
					<div id="sidebar">
						...
						<!-- Nav -->
							<nav id="nav">
								<ul>
									<li class="current_page_item"><a class="modifiable-link" href="post.php">Товары и услуги</a></li>
