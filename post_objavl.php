				...
				<!-- One -->
					<section id="one">
						<header class="major">
							...
							if (isset($_POST['ad-id'])) {$adid = $_POST['ad-id'];}
							if (empty($lead_name))
								{
									echo "<p>Не указано имя!</p>";
									echo "<p>Вернуться на страницу <a href=objavl.php> подачи объявления</a> и заполнить контактные данные</p>";
									exit;
								}
							if (empty($message))
								{
									echo "<p>Не указан текст объявления!</p>";
									echo "<p>Вернуться на страницу <a href=objavl.php> подачи объявления</a> и заполнить текст объявления</p>";
									exit;
								}	
							if (empty($adid))
								{
									echo "<p>Не указан идентификатор Ad-ID!</p>";
									echo "<p>Вернуться на страницу <a href=objavl.php> подачи объявления</a> и заполнить Ad-ID</p>";
									exit;
								}								
								if (empty($lead_email))
									{
									...
									$subject = "Скидки.РФ, Запрос размещения объявления в приложении";
									...
									if ($send == 'true')
										{
											echo "<h2>Заявка на размещение объявления получена</h2>Спасибо за интерес к нашему предложению, мы скоро с Вами свяжемся!";
											echo "<p><b><a href='index.php'></a></b></p>";
										}
										else 
											{
												echo "<p><b>Ошибка. Сообщение не отправлено! Попробуйте написать напрямую на support@sale-russia.com";
											}
								?>
