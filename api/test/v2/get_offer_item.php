<?php
	...
	function getOfferData($offer_id, $itemtype) {
		...
		if ($itemtype == "foods") {
			...
			$item_query_sql = "SELECT f.id, f.name, f.body, CONCAT ('".$server_name."',f.img1) AS img1, f.system_date_from, f.system_date_to, f.price_old, f.price_new, f.discount_size, f.add_info, f.can_complain,  c.id_name as sec_system_name, c.name as sec_name, f.offer_cat_id as category, oc.name as cat_name, f.advertiser_id, fd.name AS advertiser FROM food_offers f LEFT JOIN foods_distributors fd ON fd.id = f.advertiser_id LEFT JOIN offercategories oc ON oc.id=f.offer_cat_id LEFT JOIN categories c ON c.id = oc.section_id WHERE f.id = '".$offer_id."' LIMIT 1;";
