						...
						<div id="content-inner">
							...
							<!-- Post -->
								<article class="is-post is-post-excerpt">
									<? require("login.php");?>
									<header>
										<h2><a>Рекламодатели / Адреса</a></h2>
										
									</header>
									<? 
function show_form(){ 
        require '../config.php'; 
        $result = mysql_query("SELECT * FROM blog_addresses WHERE id = '".$_GET['id']."';", $db); 
...
<tr>
   <td> <a style="font-size:20px;text-decoration:none;" >Рекламодатель: </a></td>
   
   <td> 
    <? 


 $result2 = mysql_query("SELECT a.name, c.name as city_name FROM advertisers a LEFT JOIN cities c ON c.id = a.city_id WHERE a.id = ".$_GET['advid']." LIMIT 1", $db); 
...
<tr>
   <td> <a style="font-size:20px;text-decoration:none;" >Адрес: </a></td><td> <input style="padding:5px; background-color:#e6e6e6;font-size:15px;" type="text" name="address" value="<?=htmlspecialchars(stripslashes($row['address']));?>"  class="enter" size="50"></td></tr>
   <tr>
  ...
  <tr>
   <td> <a style="font-size:20px;text-decoration:none;" >Телефон: </a></td><td> <input style="padding:5px; background-color:#e6e6e6;font-size:15px;" type="text" name="phone" value="<?=htmlspecialchars(stripslashes($row['phone']));?>"  class="enter" size="50"></td></tr>
    <tr>
   <td> <a style="font-size:20px;text-decoration:none;" >Порядок сортировки: </a></td><td> <input style="padding:5px; background-color:#e6e6e6;font-size:15px;" type="text" name="disp_order" value="<?=htmlspecialchars(stripslashes($row['disp_order']));?>"  class="enter" size="50"></td></tr>
...
function complete(){ 
      ...
      $result = mysql_query("SELECT * FROM blog_addresses WHERE id = '".$_POST['id']."';", $db); 

     
      $row = mysql_fetch_array($result); 

      if(empty($row['id'])) 
            $query = "INSERT INTO blog_addresses 
                     (
					 advertiser_id,
					 address,
					 workhours,
            ...
            $query = "UPDATE blog_addresses SET 
      ...
      echo "<meta http-equiv='Refresh' content='0; URL=advertisers.php?id=".$_POST['advertiser_id']."&city=".$_POST['city_id_get']."'>"; 
