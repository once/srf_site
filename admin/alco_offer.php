...
function BrowseServer()
{
	...
	finder.id = 'food_thumb'; // выбираем миниатюру
	finder.popup();
...
function BrowseServer1()
{
	...
	finder.id = 'food_image'; // выбираем изображение для карточки продукта
	finder.popup();
...
function validateOfferForm() {
	...
	if (validatePlacementDate() && validateAdvertiser() && validateDates()) {
...
<? 
	...
	if (($_GET['city']) && ($_GET['city'] > 0)) {
		...
		$sqlc = mysql_query("SELECT a.id AS adv_id, a.name AS adv_name, c.name AS city_name FROM foods_distributors a LEFT JOIN cities c ON c.id = a.city_id LEFT JOIN food_distributor_categories fdc ON a.id = fdc.distributor_id WHERE fdc.section_id=10 AND a.deleted=0 ".$cityfilter." ORDER BY a.name ASC");
...
function complete(){ 
      ...
      if(empty($row['id'])) 
            ...
            $query = "UPDATE food_offers SET 
									 ...
									 offer_cat_id = '".$ALCOHOL_CATEGORY_ID."',
	  ...
	  echo "<meta http-equiv='Refresh' content='0; URL=alco_offer.php?city=".$_POST['city_id']."'>"; 
} 
function show_pages() { 
?> 
...
<? 
        require '../config.php'; ?>
		<a class="modifiable-link" href="?id=new"><h3><input style="border-radius: 0.4em;border: solid 1px #ddd;padding: 0.5em;" type="submit" value="Добавить алкогольный продукт" ></h3></a><br>
...
?>
		...
		$result = mysql_query("SELECT f.id as id, f.draft as draft, f.name as name, f.date as date,f.date1, f.paid, f.protected, f.placement_date, f.author, f.modifiedby, fd.name as fd_name, fd.id as advid, fd.draft as adv_draft, c.name as city_name, f.system_date_to FROM food_offers f LEFT JOIN foods_distributors fd ON f.advertiser_id=fd.id LEFT JOIN cities c ON c.id = fd.city_id WHERE f.offer_cat_id = ".$ALCOHOL_CATEGORY_ID." AND f.deleted=0 ".$cityfilter." ORDER BY ".$sortstring." f.id DESC;", $db);     
		
		$result_hidden_offers = mysql_query("SELECT f.id FROM food_offers f LEFT JOIN 
		foods_distributors fd ON f.advertiser_id=fd.id LEFT JOIN cities c ON c.id = fd.city_id WHERE f.draft=1  AND f.offer_cat_id = ".$ALCOHOL_CATEGORY_ID." AND f.deleted=0 ".$cityfilter." ORDER BY ".$sortstring." f.id DESC;", $db);     
		$result_hidden_by_adv = mysql_query("SELECT f.id FROM food_offers f LEFT JOIN foods_distributors fd ON f.advertiser_id=fd.id LEFT JOIN cities c ON c.id = fd.city_id WHERE (f.draft=1 OR fd.draft=1) AND f.offer_cat_id = ".$ALCOHOL_CATEGORY_ID." AND f.deleted=0 ".$cityfilter." ORDER BY ".$sortstring." f.id DESC;", $db); 
						...
						<!-- Nav -->
							<nav id="nav">
								...
								<li><a class="modifiable-link" href="post.php">Товары и услуги</a></li>
									...
									<li class="current_page_item"><a class="modifiable-link" href="alco_offer.php">Алкоголь</a></li>
