...
<? $sqlc = mysql_query("SELECT address, phone FROM blog_addresses WHERE advertiser_id = '".$row['advertiser_id']. "'  ORDER BY id ASC");

while($rowc = mysql_fetch_array($sqlc)){
	
	echo '<li>' . $rowc['address'].' </li>';
	echo '<li>' . ($rowc['address'] ? $rowc['address'] : '(без адреса) (тел. '.$rowc['phone'].')').'</li>';
}
?>
</ul>
 </td>
...
function complete(){ 
	...
	if (strlen($_POST['ext_link']) > 0) {	
	 ...
	 //// 
	  ...
	  echo "<meta http-equiv='Refresh' content='0; URL=news_offer.php?city=".$_POST['city_id']."'>"; 
} 
function show_pages() { 
?> 
...
<? 
        require '../config.php'; ?>
        <h3><a class="modifiable-link" href="?id=new"><input style="border-radius: 0.4em;border: solid 1px #ddd;padding: 0.5em;" type="submit" value="Добавить новость" ></a></h3><br>
...
if (($_GET['city']) && ($_GET['city'] > 0)) {
       ...
       // Sorting
		...
		if ($sortfield && $sortorder) {
		 ...
		 $result = mysql_query("SELECT b.id as id, b.draft as draft, b.name as name, b.date as date,b.date1,b.placement_date, b.protected, a.name as adv_name, a.id as advid, a.draft as adv_draft, c.name as city_name, b.author,b.modifiedby FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE b.deleted=0 AND cat_id=7 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
		
		$result_hidden_offers = mysql_query("SELECT b.id FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE b.draft=1 AND b.deleted=0 AND cat_id=7 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
		$result_hidden_by_adv = mysql_query("SELECT b.id FROM blog b LEFT JOIN advertisers a ON b.advertiser_id=a.id LEFT JOIN cities c ON c.id = a.city_id WHERE (b.draft=1 OR a.draft=1) AND b.deleted=0 AND cat_id=7 ".$cityfilter." ORDER BY ".$sortstring." b.id DESC", $db); 
...
<tr class="table-head"> 
 <td style="width:50px;">ID</td>
  ...
  <td ><a href="/admin/news_offer.php?city='.$_GET['city'].$sortlink.'">Срок размещения '.$sorticon.'</a></td>
    <td>Кем создано</td>
  <td>
 Создано
  </td>
						...
						<!-- Nav -->
							<nav id="nav">
								<ul>
									...
									<li class="current_page_item"><a class="modifiable-link" href="news_offer.php">Новости</a></li>
