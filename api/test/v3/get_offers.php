...
if($_GET['city'] && $_GET['city'] != 0) {
	...
	if ($_GET['type'] == "foods") {	// Продукты
		
		$strSQL = "SELECT f.id, f.name, f.metakeywords, CONCAT ('".$server_name."',f.img) as img, f.price_old, f.price_new, f.discount_size, f.order_category,f.order_section, fd.name AS advertiser, f.advertiser_id, f.offer_cat_id as category, f.pr_cat, f.exclusive, NULLIF(f.system_date_from,0) as system_date_from, NULLIF(f.system_date_to,0) as system_date_to, UNIX_TIMESTAMP(f.date1) AS modify_date  FROM food_offers f LEFT JOIN foods_distributors fd ON fd.id = f.advertiser_id LEFT JOIN cities c ON c.id = fd.city_id LEFT JOIN offercategories oc ON oc.id = f.offer_cat_id WHERE  f.draft=0 AND fd.draft=0 AND f.deleted=0 AND c.hidecitydata=0 AND ((DATE_ADD(f.system_date_to, INTERVAL 1 DAY) > NOW()) OR (ISNULL(f.system_date_to) or f.system_date_to = '0000-00-00')) AND (platform='".$platform."' OR platform='all') AND fd.city_id=" .$_GET['city']. " " . $limit . " ;";
	
	}
	
	else {	// Остальное
	
		$strSQL = "SELECT b.id, b.name, b.metakeywords, b.order_category, b.order_section,b.event_datetime,b.ext_link, b.offer_cat_id AS category, CONCAT ('".$server_name."',b.img) as img, b.discount_size, a.name as advertiser, b.pr_cat, b.exclusive, NULLIF(b.system_date_to,0) as system_date_to, UNIX_TIMESTAMP(b.date1) AS modify_date FROM blog b LEFT JOIN advertisers a ON a.id = b.advertiser_id LEFT JOIN cities c ON c.id = a.city_id WHERE b.draft=0 AND a.draft=0 AND b.deleted=0 AND c.hidecitydata=0 AND ((DATE_ADD(b.system_date_to, INTERVAL 1 DAY) > NOW()) OR (ISNULL(b.system_date_to) or b.system_date_to = '0000-00-00')) AND (platform='".$platform."' OR platform='all') AND b.cat_id=".$array_types[$_GET['type']]." ".$cityselector. " ".$categoryselector. " " . $limit .  " ;";	
