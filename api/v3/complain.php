...
switch ($offer_type) {
	...
	break;
			...
			$result = mysql_query("SELECT t1.name,t1.pr_cat,t1.paid, t2.email FROM ". $table_name ." t1 LEFT JOIN ".$adv_table. " t2 ON t2.id = t1.advertiser_id WHERE t1.id = '".$offer_id."';", $db); 
...
- Товары и услуги (Приоритет + ТОР и Платные) - рекламодателю (если есть е-меил) , sales@sale-russia.com , support@sale-russia.com
- Товары и услуги (Остальные) - рекламодателю (если есть е-меил) , support@sale-russia.com , operator02@sale-russia
- Продукты + алкоголь -  support@sale-russia.com , lady-mawa19@yandex.ru
- Остальное - support@sale-russia.com , operator02@sale-russia.com
...
if ($offer_type == 'offers') {
				...
				if (($offer_pr_cat == '3') || ($offer_pr_cat == '2') || ($offer_paid == '1')) {
					// Антон sales@sale-russia.com + Саппорт
					$to = "support@sale-russia.com , sales@sale-russia.com";
				}
				else {
					// Оператор + Саппорт
					$to = "support@sale-russia.com , operator02@sale-russia.com, lady-mawa19@yandex.ru";
				}
			...
			if ($offer_advertiser_email) {	
									...
									$send = mail ($offer_advertiser_email, $subject, $message, $headers);
...
elseif ($offer_type == 'foods') {
	// Саппорт + Мария
	$to = "support@sale-russia.com , lady-mawa19@yandex.ru";
